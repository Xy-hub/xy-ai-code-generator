...
public class AiConcurrentTest {
    ...
    @Resource
    private AiCodeGeneratorServiceFactory codeGeneratorServiceFactory;
    ...
    public void testConcurrentRoutingCalls() throws InterruptedException {
        ...
        for (int i = 0; i < prompts.length; i++) {
            ...
            threads[i] = Thread.ofVirtual().start(() -> {
                AiCodeGeneratorService aiCodeGeneratorService = codeGeneratorServiceFactory.getAiCodeGeneratorService(finalI);
                var result = aiCodeGeneratorService.routeCodeGenType(prompt);
